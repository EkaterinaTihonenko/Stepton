$(document).ready(function () {
  ...
  $(function () {
    $(
      ".burger-box,.sliding-panel-fade-screen,.sliding-panel-close, .burger2"
    ).on("click touchstart", function (e) {
      e.preventDefault();
      $(".burger-box").toggleClass("is-open");
      $(".burger2").toggleClass("is-open");
      $(
        ".sliding-panel-content, .burger2, body, .sliding-panel-fade-screen, .header_left"
...
class Dropdown {
  constructor(element, options) {
    ...
    this.modes = Array.prototype.map.call(
      this.choices,
      (choice) => choice.dataset["mode"]
    );
  ...
  toggleExpanded() {
    if (
      !this.element.classList.contains(this.classExpanded) ||
      !this.element.classList.contains("js-dropdown-expanded")
    ) {
      this.setExpanded();
    } else {
      this.unsetExpanded();
  ...
  colorize(mode) {
    Array.prototype.forEach.call(this.modes, (item) => {
      this.toggler.classList.remove(`dropdown__header_${item}`);
    });
  ...
  setIcon(mode) {
    Array.prototype.forEach.call(this.modes, (item) => {
      this.icon.classList.remove(`fa-${item}`);
    });
